...
const TransaksiMasuk = () => {
  ...
  const productOptions = produk.map((p) => ({
    value: p.id,
    label: p.nama_produk,
    sublabel: `Rp ${Number(p.harga).toLocaleString("id-ID")}`,
  }));
  ...
  const handleEditClick = (item) => {
    ...
    setSelectedProduk(prod ? { value: prod.id, harga: prod.harga } : null);
    ...
    <div className="flex flex-1 flex-col gap-4 p-6">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <h2 className="text-3xl font-bold tracking-tight">Transaksi Masuk</h2>
          <p className="text-sm text-muted-foreground">
            Catat dan pantau pasokan stok barang yang masuk ke koperasi.
          </p>
        </div>
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold">Catat Transaksi Masuk</DialogTitle>
          </DialogHeader>
              ...
              <div className="grid gap-2">
                ...
                <SearchableSelect
                  ...
                  onValueChange={(value) => {
                    ...
                    setSelectedProduk(selected ? { value: selected.id, harga: selected.harga } : null);
                  }}
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold">Ubah Transaksi Masuk</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent className="sm:max-w-[420px]">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold">Hapus Transaksi Masuk?</DialogTitle>
          </DialogHeader>
          <div className="py-2 text-sm text-zinc-500">
            Apakah Anda yakin ingin menghapus transaksi masuk ini? Tindakan ini akan secara otomatis menyesuaikan kembali jumlah stok produk tersebut.
          </div>
          <div className="flex justify-end gap-3 pt-4 border-t">
