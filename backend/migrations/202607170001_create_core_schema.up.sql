...
CREATE TABLE IF NOT EXISTS pengguna (
  ...
  CONSTRAINT chk_pengguna_role
    CHECK (role IN ('admin', 'staff'))
);

CREATE UNIQUE INDEX IF NOT EXISTS uq_pengguna_username_lower
  ON pengguna (LOWER(BTRIM(username)));

CREATE INDEX IF NOT EXISTS idx_pengguna_role_active
  ON pengguna (role, is_active);
...
CREATE TABLE IF NOT EXISTS produk (
  ...
  CONSTRAINT chk_produk_kode_not_empty
...
CREATE INDEX IF NOT EXISTS idx_produk_stok_kritis
  ON produk (stok, stok_minimum);
...
CREATE TABLE IF NOT EXISTS dataset_mingguan (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT uq_dataset_mingguan_produk_period
  ...
  CONSTRAINT chk_dataset_mingguan_minggu
    CHECK (minggu_ke BETWEEN 1 AND 4),
  CONSTRAINT chk_dataset_mingguan_period_not_empty
    CHECK (BTRIM(period_label) <> ''),
  CONSTRAINT chk_dataset_mingguan_total_nonnegative
    CHECK (total_penjualan >= 0)
);

CREATE INDEX IF NOT EXISTS idx_dataset_mingguan_produk_periode
